...
class SAM2ImagePredictor:
    def __init__(
        ...
        **kwargs,
        ...
        # set _is_batch = True for batching 
        self._is_batch = True
    ...
    def predict(
        ...
        # here you need to prep the prompts
        mask_input, unnorm_coords, labels, unnorm_box = self._prep_prompts(
    ...
    ''''''''''
    This is so the user can get the sparse and dense embeddings 
    '''
    def predict_sparse_and_dense_embeddings(
        self,
        point_coords: Optional[np.ndarray] = None,
        point_labels: Optional[np.ndarray] = None,
        box: Optional[np.ndarray] = None,
        ...
        # here you need to prep the prompts, so we need to call this before 
        # we call get_sparse_and_dense_embeddings, which then calls the sam_prompt_encoder
        mask_input, unnorm_coords, labels, unnorm_box = self._prep_prompts(
            point_coords, point_labels, box, mask_input, normalize_coords
        )

        sparse_embeddings, dense_embeddings = self.get_sparse_and_dense_embeddings(
            unnorm_coords,
            labels,
            unnorm_box,
            mask_input,
    ...
    @torch.no_grad()
    def get_sparse_and_dense_embeddings(
        self,
        point_coords: Optional[torch.Tensor],
        point_labels: Optional[torch.Tensor],
        boxes: Optional[torch.Tensor] = None,
...
#########################################################################################################################################################################
    ...
    '''
    def get_sparse_and_dense_embeddings_batch(
        self,
        point_coords_batch: List[np.ndarray] = None,
        point_labels_batch: List[np.ndarray] = None,
        box_batch: List[np.ndarray] = None,
        ...
        for img_idx in range(len(image_embeddings)):
            ...
            if (box_batch[img_idx].shape != (0,4)):
                ...
                f_embeddings = torch.mean(torch.cat((image_embeddings_for_meanpool,dense_embeddings),0), 0).unsqueeze(0)
    ...
    @torch.no_grad()
    def get_sparse_and_dense_embeddings_batched(
        self,
        point_coords: Optional[torch.Tensor],
        point_labels: Optional[torch.Tensor],
        boxes: Optional[torch.Tensor] = None,
        ...
        # sparse_embeddings and dense_embeddings
        return sparse_embeddings, dense_embeddings
